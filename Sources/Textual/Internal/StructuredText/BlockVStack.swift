...
extension StructuredText {
  ...
  fileprivate struct BlockLayoutView<Content: View>: View {
    ...
    var body: some View {
      // Layout values set by .textual.blockSpacing() propagate up from the
      // block styles synchronously. We only need to override for list items.
      if listItemSpacingEnabled {
        content
          .layoutValue(key: BlockSpacingKey.self, value: resolvedListItemSpacing)
      } else {
        content
      }
    }
  ...
  fileprivate struct BlockVStackLayout: Layout {
    ...
    func sizeThatFits(
      ...
      cache.sizes = subviews.map { view in
        view.sizeThatFits(.init(width: proposal.width, height: nil))
      }

      for viewSize in cache.sizes {
    ...
    func placeSubviews(
      ...
      for (index, view) in zip(subviews.indices, subviews) {
        ...
        let viewSize = index < cache.sizes.count ? cache.sizes[index] : view.sizeThatFits(viewProposal)
